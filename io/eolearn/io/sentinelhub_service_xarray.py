...
class SentinelHubOGCInputXarray(EOTask):
    """ Task for creating EOPatch and filling it with data using Sentinel Hub's OGC request.

    :param layer: the preconfigured SentinelHub layer to be added to EOPatch's RASTER feature.
    :type layer: str
    ...
    def __init__(self, layer, feature=None, valid_data_mask_feature='IS_DATA', service_type=None, data_source=None,
        ...
        self.feature_type, self.feature_name = next(self._parse_features(layer if feature is None else feature,
                                                                         default_feature_type=FeatureType.RASTER)())
    ...
    def execute(self, eopatch=None, bbox=None, time_interval=None):
        ...
        # TODO: save also dates in time dim  
        ...
        self._add_data(eopatch, np.asarray(images), request_dates if download_frames is None else download_frames, request_params)
        self._add_meta_info(eopatch, request_params, service_type)

        return eopatch


class SentinelHubWMSInputXarray(SentinelHubOGCInputXarray):
    """
...
class SentinelHubWCSInputXarray(SentinelHubOGCInputXarray):
    """
...
class S2L1CWMSInputXarray(SentinelHubWMSInputXarray):
    """
...
class S2L1CWCSInputXarray(SentinelHubWCSInputXarray):
    """
...
class L8L1CWMSInputXarray(SentinelHubWMSInputXarray):
    """
...
class L8L1CWCSInputXarray(SentinelHubWCSInputXarray):
    """
...
class S2L2AWMSInputXarray(SentinelHubWMSInputXarray):
    """
...
class S2L2AWCSInputXarray(SentinelHubWCSInputXarray):
    """
...
class S1IWWMSInputXarray(SentinelHubWMSInputXarray):
    """
...
class S1IWWCSInputXarray(SentinelHubWCSInputXarray):
    """
...
class DEMWMSInputXarray(SentinelHubWMSInputXarray):
    """
...
class DEMWCSInputXarray(SentinelHubWCSInputXarray):
    """
    Adds DEM to DATA_TIMELESS EOPatch feature.
    """
    def __init__(self, layer, feature=None, **kwargs):
...
class AddSen2CorClassificationFeatureXarray(SentinelHubOGCInputXarray):
    """
